...
The review used `agents/instruction.md`, the [OCT paper](https://arxiv.org/html/2511.01689v1), the checked-out [original implementation](https://github.com/maiush/OpenCharacterTraining) (distillation/introspection source and training scripts), and the [official Qwen3.5-9B model card](https://huggingface.co/Qwen/Qwen3.5-9B). OCT's original code uses LoRA and constitution-conditioned introspection; this pilot deliberately substitutes full-parameter updates, an M0 teacher, and a much smaller prompt/introspection scale. Those are adaptations, not reproductions of the paper's empirical claims.
